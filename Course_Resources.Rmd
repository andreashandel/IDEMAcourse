...
After the due date for each quiz, I will place solution files to the DSAIDE exercises in a Dropbox folder. The solutions are fairly detailed and also show you how you can use `R` commands (instead of the graphical interface) to do all the tasks. I highly recommend checking out the solutions (even if you get all answers correct). The link to the dropbox folder is pinned in the _Announcement_ channel on Slack.
