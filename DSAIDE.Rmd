...
**Note: Unfortunately, the latest version of the DSAIDE package didn't make it to CRAN in time (they are on vacation). Therefore, you need to install DSAIDE from Github with the `remotes` package. Follow the 2nd option (installing from GitHub) as described in the [_Installation_ section here](https://ahgroup.github.io/DSAIDE/articles/DSAIDE.html). Make sure you have version 0.8.6.**
...
A menu should open in your browser, from which you can explore different models/apps. **Make sure the version and date shown at the top is recent and corresponds to the version you are asked to have. Right now this is version 0.8.6.**
...
_This source cover more or less the same material that you just read, so checking it out is optional._
